...
from utils.resolve_files import PatientResolver, get_raw_text
...
def get_data(df):
    ...
    for idx, (_, text_file) in enumerate(data):
...
def fix_dates(df):
    ...
    df.loc[:, "final_date"] = df["final_date"].apply(_fix)
    df.loc[:, "admission_date"] = df["admission_date"].apply(_fix)
    df.loc[:, "covid_test_date"] = df["covid_test_date"].apply(_fix)
...
if __name__=="__main__":
    for fname in glob.glob(f"{ROOT_DIR}/*/zestawienie*.xlsx"):
        ...
        print("Basic data:")
        print(basic_data_df)
        upload("https://localhost", basic_data_df)
